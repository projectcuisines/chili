...
#                pdb.set_trace()
        ...
        #melt_vol_frac will be exported as PCH4 for now
    ...
    XFe2O3 =XFeO * (XFe2O3_over_XFeO)                     
    '''# OLD VERSION --- delete once new version works
    #XFeO = New_Total_Fe / (1 + 2 * 10**(log_XFe2O3_over_XFeO))     #for Sossi parameterization
    #XFe2O3 =XFeO * 10**(log_XFe2O3_over_XFeO)                      #for Sossi parameterization
    XFeO = New_Total_Fe / (1 + 2 * np.exp(log_XFe2O3_over_XFeO))    #for Kress & Carmichael fO2 param
    XFe2O3 =XFeO * np.exp(log_XFe2O3_over_XFeO)                     #for Kress & Carmichael fO2 param '''
    ...
    #O_solid_res = (2*y_out[13,:]/0.012) +  (0.5*y_out[0,:]/0.001) + (y_out[5,:]*0.5*16/(56+1.5*16))/0.016
        ...
        '''# OLD VERSION
        if Metal_sink_switch==0:
            #[log_XFe2O3_over_XFeO,new_Total_Fe] = Iron_speciation_smooth_Sossi(total_y[22,kk]/1e5,Pressure_surface[kk]*1e5,total_y[8,kk],Total_Fe_mol_fraction_start) #Sossi
            [log_XFe2O3_over_XFeO,new_Total_Fe] = Iron_speciation_smooth(total_y[22,kk]/1e5,Pressure_surface[kk]*1e5,total_y[8,kk],Total_Fe_mol_fraction_start) # Kress & Carmich
            #XFeO = new_Total_Fe / (1 + 2 * 10**(log_XFe2O3_over_XFeO))     #for Sossi parameterization
            #XFe2O3 =XFeO * 10**(log_XFe2O3_over_XFeO)                      #for Sossi parameterization
            XFeO = new_Total_Fe / (1 + 2 * np.exp(log_XFe2O3_over_XFeO))    #for old Kress & Carmich fO2 parameterization
            XFe2O3 =XFeO * np.exp(log_XFe2O3_over_XFeO)                     #for old Kress & Carmich fO2 parameterization
            F_FeO_ar[kk] = M_melt*XFeO * (56.0 + 16.0) / m_sil 
            F_FeO1_5_ar[kk] = M_melt*XFe2O3 * (2*56.0 + 3*16.0) / m_sil 
            F_Fe_ar[kk] = M_melt*(Total_Fe_mol_fraction_start - new_Total_Fe)*56/m_sil
        else:
            #[log_XFe2O3_over_XFeO,new_Total_Fe] = Iron_speciation_smooth2_Sossi(total_y[22,kk]/1e5,Pressure_surface[kk]*1e5,total_y[8,kk],total_y[17,kk])   #Sossi
            [log_XFe2O3_over_XFeO,new_Total_Fe] = Iron_speciation_smooth2(total_y[22,kk]/1e5,Pressure_surface[kk]*1e5,total_y[8,kk],total_y[17,kk])     #Kress & Carmichael
            #XFeO = new_Total_Fe / (1 + 2 * 10**(log_XFe2O3_over_XFeO))      #for Sossi parameterization
            #XFe2O3 =XFeO * 10**(log_XFe2O3_over_XFeO)                       #for Sossi parameterization
            XFeO = new_Total_Fe / (1 + 2 * np.exp(log_XFe2O3_over_XFeO))   #for old Kress & Carmich fO2 parameterization
            XFe2O3 =XFeO * np.exp(log_XFe2O3_over_XFeO)                    #for old Kress & Carmich fO2 parameterization 
