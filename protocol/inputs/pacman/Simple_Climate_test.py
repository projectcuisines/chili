...
### UDPATED SINCE PAPER BCEAUSE not Nelder_mead is maximize, others are minimize!!
@jit(nopython=True)
def funTs_new_fast_ps(Ts,H2O_input,CO2_input,CO_input,Te_input,H2_input,ASR):
    Ts_in= Ts[0]
    [OLR,EM_ppCO2_o,EM_pH_o,ALK,Mass_oceans_crude,DIC_check,Mass_CO2_atmo] = correction(Ts_in,Te_input,H2O_input,CO2_input,CO_input,H2_input,6371000.0,9.8,5e-5,1e18,0.028,1e-6 )
    #print(Ts,OLR,ASR,(ASR-OLR)**2)
    return (ASR-OLR)**2
...
def solve_Tsurf(MH2O_in,MCO2_in,MCO_in,Te_in,MH2_in,ASR_in,Tguess):
    ...
    if new_abs > 0.1:
        ...
        if new_abs > 1.0:#0.1:
            ...
            ace3= optimize.minimize(funTs_new_fast_ps,x0=251,args = (MH2O_in,MCO2_in,MCO_in,Te_in,MH2_in,ASR_in),method='L-BFGS-B',bounds = ((250,4000),))
            #print(ace3)
            if abs(ace3.fun) <  new_abs:
                ...
                if new_abs > 1.0:#0.1:
                    ...
                    ace4= optimize.minimize(funTs_new_fast_ps,x0=rand_start,args = (MH2O_in,MCO2_in,MCO_in,Te_in,MH2_in,ASR_in),method='L-BFGS-B',bounds = ((150,4000),))
                    #print(ace4)
