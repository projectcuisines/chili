...
# -------------------------------------------------------------------------
def build_pkl_path(Tsurf, H, C, O=None, N=None, He=None, P=None, S=None,
	fO2_calculation=None, fO2_buffer=None, fO2_log_dev=None):
	"""
	Build the expected pkl file path for a given Tsurf and volatile inputs,
	matching MAGMAVOL_2's actual saved filename convention exactly.

	All parameters default to None, resolving to the corresponding
	module-level constant at call time (same None-sentinel pattern used
	throughout this script). This means reassigning module-level constants
	after import takes effect without needing to pass them explicitly.

	In TARGET_FO2 mode: appends _{fO2_calculation}_{fO2_buffer}_{fO2_log_dev}
	to the filename, matching MAGMAVOL_2's pkl save logic.
	In MASS_BALANCE mode: omits that suffix entirely, also matching MAGMAVOL_2.
	fO2_buffer/fO2_log_dev are silently ignored in MASS_BALANCE mode.
	"""
	if O              is None: O              = globals()['O']
	if N              is None: N              = globals()['N']
	if He             is None: He             = globals()['He']
	if P              is None: P              = globals()['P']
	if S              is None: S              = globals()['S']
	if fO2_calculation is None: fO2_calculation = FO2_CALCULATION
	if fO2_buffer     is None: fO2_buffer     = FO2_BUFFER
	if fO2_log_dev    is None: fO2_log_dev    = FO2_LOG_DEV
...
def run_chili_single(planet='trappist1e', tau_key='tau3', hot_or_cold='hot',
	use_seeded_O=True, fO2_calculation=None):
	"""
	...
	use_seeded_O   : if True (default), O is seeded from the evolutionary
	                 model's massO_atm(kg). If False, O is forced to 0.
	fO2_calculation: 'TARGET_FO2' or 'MASS_BALANCE'. Defaults to None,
	                 resolving to the module-level FO2_CALCULATION at call time.
	"""
	if fO2_calculation is None:
...
# -------------------------------------------------------------------------
# STEP 1b: Run full temperature grid seeded from the same evolutionary
#          model inputs as run_chili_single — for a specific (planet,
#          tau_key, hot_or_cold) combination, using H/C/O/fO2_dev derived
#          from the evolutionary model's row at that tau rather than the
#          fixed Table 2/4 values used by run_full_grid.
# -------------------------------------------------------------------------
def run_chili_grid(planet, tau_key, hot_or_cold,
	fO2_calculation=None, use_seeded_O=True):
	"""
	Run MAGMAVOL across the full temperature grid (t_arr_run) for a
	specific (planet, tau_key, hot_or_cold) combination, seeding H/C/O
	(bar) and FO2_LOG_DEV from the selected evolutionary model's row at
	that tau — exactly as run_chili_single does, but sweeping over the
	full temperature grid rather than a single T_surf.

	On each temperature point:
	  - If a pkl already exists in the reservoir, skip.
	  - If MAGMAVOL does not converge (pkl not saved), log a warning.
	  - MAGMAVOL_2 handles convergence/reservoir saving internally.

	After the full sweep, writes a compo .dat file and calls
	interpolate_compo_file() automatically to fill any missing rows.
	...
	log.info(f"=== CHILI grid run: {planet} {tau_key} ({hot_or_cold}), "
		f"seeded from model '{model_name}' ===")
	log.info(f"  Seed row: t_used = {seed['t_used']:.2f} yr "
		f"(target tau = {tau_yr:.0e} yr)")
	log.info(f"  Seeded volatile inputs: H = {H} bar, C = {C} bar, O = {O} bar")
	log.info(f"  Seeded FO2_LOG_DEV = {fo2_dev}  (source: {seed['fo2_source']})")
	log.info(f"  FO2_CALCULATION = {fO2_calculation}")
	log.info(f"  T range: {T_MIN} to {T_MAX} K in {T_STEP} K steps "
		f"({len(t_arr)} points)")
	log.info(f"  Running high to low temperature")

	for Tsurf in t_arr_run:
		pkl_path = build_pkl_path(Tsurf, H, C, O=O, N=N, He=He, P=P, S=S,
			fO2_calculation=fO2_calculation, fO2_buffer=FO2_BUFFER,
			fO2_log_dev=fo2_dev)
		...
		if not os.path.exists(pkl_path):
			log.info(f"NOT CONVERGED at T={Tsurf:.0f} K")
	...
	sp_list = list(data['atoms_molar_fractions'].keys())

	for num, (T, p, mf) in enumerate(zip(present_temps, ptot, arr_list)):
		sp_temp = np.zeros(len(species))
		for sp_num, sp in enumerate(species):
			check = [i for i, s in enumerate(sp_list) if s == sp]
			val_temp = list(mf.values())
			if check:
	...
	# compo filename encodes mode, seed, and volatile inputs
	if fO2_calculation == 'MASS_BALANCE':
		compo_file = os.path.join(
			CHILI_DIR,
			f'compo_chili_pH={H}_pC={C}_pO={O}'
			f'_{model_name}_{tau_key}_{hot_or_cold}_mass_balance_new.dat'
		)
	else:
		compo_file = os.path.join(
			CHILI_DIR,
			f'compo_chili_pH={H}_pC={C}_pO={O}_fug={fo2_dev}'
			f'_{model_name}_{tau_key}_{hot_or_cold}_new.dat'
		)
...
def run_full_grid(planet):
	...
	for Tsurf in t_arr_run:
		pkl_path = build_pkl_path(Tsurf, H, C, O=O, N=N, He=He, P=P, S=S,
			fO2_calculation=FO2_CALCULATION, fO2_buffer=FO2_BUFFER, fO2_log_dev=FO2_LOG_DEV)
		if os.path.exists(pkl_path):
			log.info(f"  T={Tsurf:.0f} K: pkl already exists, skipping.")
			continue
		log.info(f"  T={Tsurf:.0f} K: running MAGMAVOL ...")
...
if __name__ == '__main__':
	...
	# -------------------------------------------------------------------------
	# --- trappist1e ---
	data = run_chili_single(planet='trappist1e', tau_key='tau3', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1e', tau_key='tau3', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1e', tau_key='tau4', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1e', tau_key='tau4', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1e', tau_key='tau6', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1e', tau_key='tau6', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')

	# --- trappist1b ---
	data = run_chili_single(planet='trappist1b', tau_key='tau3', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1b', tau_key='tau3', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1b', tau_key='tau4', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1b', tau_key='tau4', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1b', tau_key='tau6', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1b', tau_key='tau6', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')

	# --- trappist1alpha ---
	data = run_chili_single(planet='trappist1alpha', tau_key='tau3', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1alpha', tau_key='tau3', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1alpha', tau_key='tau4', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1alpha', tau_key='tau4', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1alpha', tau_key='tau6', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='trappist1alpha', tau_key='tau6', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')

	# --- earth ---
	data = run_chili_single(planet='earth', tau_key='tau3', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='earth', tau_key='tau3', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='earth', tau_key='tau4', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='earth', tau_key='tau4', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='earth', tau_key='tau5', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='earth', tau_key='tau5', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='earth', tau_key='tau6', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='earth', tau_key='tau6', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')

	# --- venus ---
	data = run_chili_single(planet='venus', tau_key='tau3', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='venus', tau_key='tau3', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='venus', tau_key='tau4', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='venus', tau_key='tau4', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='venus', tau_key='tau5', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='venus', tau_key='tau5', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='venus', tau_key='tau6', hot_or_cold='hot', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')
	data = run_chili_single(planet='venus', tau_key='tau6', hot_or_cold='cold', use_seeded_O=True, fO2_calculation = 'MASS_BALANCE')

	# -------------------------------------------------------------------------
	# Step 1b: run seeded temperature grid and write a compo file.
	# Each (planet, tau_key, hot_or_cold) combination produces its own compo
	# file, since H/C/O differ between hot and cold cases within a tau.
	# Interpolation of missing temperatures runs automatically at the end.
	# -------------------------------------------------------------------------
	# --- trappist1e ---
	# run_chili_grid(planet='trappist1e', tau_key='tau3', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1e', tau_key='tau3', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1e', tau_key='tau4', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1e', tau_key='tau4', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1e', tau_key='tau6', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1e', tau_key='tau6', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')

	# --- trappist1b ---
	# run_chili_grid(planet='trappist1b', tau_key='tau3', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1b', tau_key='tau3', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1b', tau_key='tau4', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1b', tau_key='tau4', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1b', tau_key='tau6', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1b', tau_key='tau6', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')

	# --- trappist1alpha ---
	# run_chili_grid(planet='trappist1alpha', tau_key='tau3', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1alpha', tau_key='tau3', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1alpha', tau_key='tau4', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1alpha', tau_key='tau4', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1alpha', tau_key='tau6', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='trappist1alpha', tau_key='tau6', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')

	# --- earth ---
	# run_chili_grid(planet='earth', tau_key='tau3', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='earth', tau_key='tau3', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='earth', tau_key='tau4', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='earth', tau_key='tau4', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='earth', tau_key='tau5', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='earth', tau_key='tau5', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='earth', tau_key='tau6', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='earth', tau_key='tau6', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')

	# --- venus ---
	# run_chili_grid(planet='venus', tau_key='tau3', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='venus', tau_key='tau3', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='venus', tau_key='tau4', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='venus', tau_key='tau4', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='venus', tau_key='tau5', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='venus', tau_key='tau5', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='venus', tau_key='tau6', hot_or_cold='hot', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')
	# run_chili_grid(planet='venus', tau_key='tau6', hot_or_cold='cold', use_seeded_O=False, fo2_calculation = 'TARGET_FO2')

	# -------------------------------------------------------------------------
	# Step 2: run fixed-input temperature grid (Table 2/4 H/C, planet-only).
