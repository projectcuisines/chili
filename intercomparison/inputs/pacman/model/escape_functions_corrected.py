...
@jit(nopython=True)
def better_diffusion(fH2O,T,g,fCO2,fO2,fN2): # diffusion H2O through CO2 and N2
    pO2 = fO2
    pN2 = fN2
    pCO2 = fCO2
    ## Units conversion 
...
def better_H_diffusion(fH,T,g,fCO,fO,fN2): # diffusion H2O thrugh CO2 and N2
    ...
    Hn = (8.314*T)/(Mn*g)
    H_H = (8.314*T)/(0.001*g)       #molar mass of atomic H= 0.001
...
def find_epsilon(T,RE,ME,FXUV, XO, XH, XC,epsilon_init,mixparam):
    ...
    Rp = RE * 6.371e6       #planet raduis in m
    Mp = ME * 5.972e24      #planet mass in kg
    g = G* Mp / (Rp**2)     #grav accel at surface
    delPHI = G*Mp/Rp        #grav potential at surface
    mass_loss = epsilon_init * FXUV  / (4.0*delPHI)   #mass loss rate    
    mi = mH         #mass of hydrogen atom
    mj = 16.0*mH    #mass of oxygen
    
    bij = 100.0*4.8e17*(T**0.75) #100 * cm-1 s-1 bH,O  \ temp dependent collision parameter for H O interactions
    fj = XO/XH          

    Fi_limit  =  g*(mj - mi) * bij   / ( kB * T * (1+fj))  #max flux of particles escaping
    true_flux = Fi_limit * mi       #actual flux of hydrogen escape
